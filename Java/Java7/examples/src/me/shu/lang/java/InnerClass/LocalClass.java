 ...
 * 我们知道，局部变量在其所属的代码段（譬如某个函数）执行完毕后就会被回收，而一个局部类的实例却可以在其类定义所属代码段执行完毕后依然存在，如果它可操控非final的局部变量，用户就可以通过该实例修改已不存在的局部变量，无意义。
 * Java采用了一种copy local final variable的方式来实现，也就是说把定义为final的局部变量拷贝过来用，而引用的也可以拿过来用，只是不能重新赋值。
 * 从而造成了可以access local final variable的假象</br></br>
 * 5. 如示例代码所示，<b>内部类只在定义它的代码段中可见</b>，不能在它所属代码段之外的代码中使用；因此也就没有public/private/default权限修饰符（无意义）。
