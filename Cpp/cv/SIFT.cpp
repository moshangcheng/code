...
MySIFT::MySIFT()
{
	keypoints=NULL;      //用于临时存储特征点的位置等
	keyDescriptors=NULL; //用于最后的确定特征点以及特征描述字
	storage = 0; 	
	/////////////////////////////
	keypoints2 = NULL;
	keyDescriptors2 = NULL;
	Frame1Keys = NULL;// 存储第一帧的关键特征点
	Frame2Keys = NULL;
}
int MySIFT::SiftMatch(IplImage* src,IplImage* src2)
{
	CvPoint2D32f tpoint;
  //声明当前帧IplImage指针
 // IplImage* src = NULL; 
...
/////////////////////////////////////
 ...
 // IplImage* src2 = NULL;  
  ...
  IplImage* grey_im2 = NULL;
  IplImage* DoubleSizeImage2 = NULL;
  CvMat* image2Mat = NULL;
...
/////////////////////////////////////  
  ...
  cvSmooth(src, src, CV_BLUR, 5,3, 0, 0);
//	IplConvKernel* ele = cvCreateStructuringElementEx(5,5,2,2,CV_SHAPE_CROSS,0);
//	cvErode(src, src, ele, 1);
  cvSmooth(src2, src2, CV_BLUR, 5,3, 0, 0);
  //为图像分配内存
  ///////////////////////////////////////////
...
//  cvResizeWindow("image1", 2*(image1->width), 2*(image1->height) );
  ...
  Frame1Keys = keyDescriptors;
  Keypoint temp = Frame1Keys->next;
  printf("%f,   %f \n", Frame1Keys->row, Frame1Keys->col);
  printf("%f,   %f \n", temp->row, temp->col);
  cvWaitKey(0);
 ...
 /////////////////////////////////////////////////////////////////////////////////////////
  keypoints=NULL;      //用于临时存储特征点的位置等
   keyDescriptors=NULL; //用于最后的确定特征点以及特征描述字
    image2 = cvCreateImage(cvSize(src2->width, src2->height),  IPL_DEPTH_8U,3);
////////////////
   grey_im2 = cvCreateImage(cvSize(src2->width, src2->height),  IPL_DEPTH_8U,1);
  DoubleSizeImage2 = cvCreateImage(cvSize(2*(src2->width), 2*(src2->height)),  IPL_DEPTH_8U,3);

  //为图像阵列分配内存，假设两幅图像的大小相同，tempMat跟随image1的大小
  image2Mat = cvCreateMat(src2->height, src2->width, CV_32FC1);
  //转化成单通道图像再处理
  cvCvtColor(src2, grey_im2, CV_BGR2GRAY);
  //转换进入Mat数据结构,图像操作使用的是浮点型操作
  cvConvert(grey_im2, image2Mat);
  
  t = (double)cvGetTickCount();
  //图像归一化
  cvConvertScale( image2Mat, image2Mat, 1.0/255, 0 );

  dim = min(image2Mat->rows, image2Mat->cols);
  numoctaves = (int) (log((double) dim) / log(2.0)) - 2;    //金字塔阶数
  numoctaves = min(numoctaves, MAXOCTAVES);

  //第一步，预滤波除噪声，建立金字塔底层
  tempMat = ScaleInitImage(image2Mat) ;
  //第二步，建立Guassian金字塔和DOG金字塔
  Gaussianpyr = BuildGaussianOctaves(tempMat) ;
  ...
  for (int i=0; i<numoctaves;i++)
  {
  	  if (i==0)
      {
          ...
          for (int j=2;j<SCALESPEROCTAVE+3;j++)
              mosaicHorizen1=MosaicHorizen( mosaicHorizen1, (Gaussianpyr[0].Octave)[j].Level );
		 for (int j=0;j<NUMSIZE;j++)//进行NUMSIZE次的下采样
              mosaicHorizen1=halfSizeImage(mosaicHorizen1);
      }
      else if (i==1)
      {
...
*/
  ...
  */
  ...
  printf("the keypoints number are %d ;\n", keycount);
  cvCopy(src2,image2,NULL);
  DisplayKeypointLocation( image2 ,Gaussianpyr);
...
//  cvResizeWindow("image1", 2*(image1->width), 2*(image1->height) );
  ...
  ///////////////////////////////////////////////////////////////
  Frame2Keys = keyDescriptors;
  Keypoint tempxx = Frame2Keys->next->next;
  printf("%f,   %f \n", Frame2Keys->row, Frame2Keys->col);
  printf("%f,   %f \n", tempxx->row, tempxx->col);
  ///////////////////////////////////////////////////////////////
  cvWaitKey(0);
///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //匹配过程
	nMatch = 0;
	Keypoint minPoint1 = NULL;
	Keypoint minPoint2 = NULL;
	Keypoint keys1 = NULL;
	Keypoint keys2 = NULL;
	Keypoint tempMinKey2 = NULL;
	Keypoint tempMinKey1 = NULL;
	double tempDistance, minDistance1, minDistance2; 
	keys1 = Frame1Keys;
	keys2 = Frame2Keys;
	tempDistance = 0;
	...
	while (keys1)
	{
		...
		while (keys2)
		{
			...
			for (int i = 0; i < 128; i ++)
			{
				tempDistance += pow(keys1->descrip[i] - keys2->descrip[i], 2);
			}
			if (tempDistance < minDistance1)
			{
				tempMinKey2 = tempMinKey1;
				tempMinKey1 = keys2;
		...
		if (minDistance1 / minDistance2 <= 0.14)
		{
			printf("%lf, %lf\n",minDistance1, minDistance2);
			printf("row1 = %f, col1 = %f; row2 = %f, col2 = %f\n",
				keys1->row, keys1->col, tempMinKey1->row, tempMinKey1->col);
			tpoint.x = keys1->col;
			tpoint.y = keys1->row;
			MatchPoints[0].push_back(tpoint);
			tpoint.x = tempMinKey1->col;
			tpoint.y = tempMinKey1->row;
			MatchPoints[1].push_back(tpoint);
			nMatch ++;
			cvCircle(src, cvPoint(keys1->col, keys1->row), 2, CV_RGB(255,0,0),2);
			cvCircle(src2, cvPoint(tempMinKey1->col, tempMinKey1->row), 2, CV_RGB(255,0,0),2);
		}
		keys1 = keys1->next;
	}
	cvShowImage("image1",src);
...
CvMat* MySIFT::doubleSizeImage2(CvMat * im) 
{
	...
	/*//对于不同的点情况，采用不同的插值形式，注意相应像素坐标的表达形式，如下的i，j的初值情况不同
	  A B C
	  E F G
	  H I J
	...
	*/
	...
	for ( j = 0; j < h; j += 2)  //插值改变相应像素值，这里尺度+2对应的那些像素
		for ( i = 1; i < w - 1; i += 2)
...
//得到向量的欧式长度，2-范数
float MySIFT::GetVecNorm( float* vec, int dim )
{
	float sum=0.0;
...
{
//	int dim = (int) max<double>(3.0f, GAUSSKERN * sigma);
    int dim = (int) max<double>(3.0f, 2.0 * GAUSSKERN *sigma + 1.0f);
	// make dim odd
	if (dim % 2 == 0)
		dim++;
...
ImageOctaves* MySIFT::BuildGaussianOctaves(CvMat * image) 
{
 ...
 //   initial_sigma = sqrt(SIGMA * SIGMA - INITSIGMA * INITSIGMA * 4);
	...
	{   
		...
		{
			(octaves[i].Octave)[0].levelsigma = (octaves[i-1].Octave)[SCALESPEROCTAVE].levelsigma;//等于上一阶梯的第SCALESPEROCTAVE层对应的sigma值
...
//			}		
            ...
            //产生DOG层
            cvSub( ((octaves[i].Octave)[j]).Level, ((octaves[i].Octave)[j-1]).Level, temp, 0 );//两层作差
	...
	{        
		...
		{  
		    ...
		    for (int m=dim;m<((DOGoctaves[i].row)-dim);m++) 
				...
				{					
					if ( fabs(ImLevels(i,j,m,n))>= CONTRAST_THRESHOLD )//差值阈值，DOG中的值设定了一阈值
					{
						...
						{   
							...
							if ( fabs(ImLevels(i,j,m,n))>= CONTRAST_THRESHOLD )
							{
                                ...
                                */
								...
								Dxy = ImLevels(i,j,m-1,n-1) + ImLevels(i,j,m+1,n+1) - ImLevels(i,j,m+1,n-1) - ImLevels(i,j,m-1,n+1);//??为什么是这样的呢？推导时有个四倍的关系
                                //Dxy = 0.25*(ImLevels(i,j,m-1,n-1) + ImLevels(i,j,m+1,n+1) - ImLevels(i,j,m+1,n-1) - ImLevels(i,j,m-1,n+1));//我觉得是这样的式子,效果也不好
								// Dxy = ImLevels(i,j,m+1,n+1) + ImLevels(i,j,m,n) - ImLevels(i,j,m,n+1) - ImLevels(i,j,m+1,n);//我觉得也可以是这个表达式,但用这个式子时，效果明显差很多
								Tr_H = Dxx + Dyy;
	...
	{        
		...
		{  
		    ...
		    for (int m=1;m<(GaussianPyr[i].row-1);m++) 
				...
				{
					//计算幅值
					TEMPMAT1(m,n) = 0.5*( ImLevels(i,j,m,n+1)-ImLevels(i,j,m,n-1) );  //dx  //因为dx有两种表示，故相加后得到这样的形式
...
void MySIFT::AssignTheMainOrientation(int numoctaves, ImageOctaves *GaussianPyr,ImageOctaves *mag_pyr,ImageOctaves *grad_pyr)
{
    ...
    for (int i=0;i<36;i++)
	   hist_orient[i]=-PI+i*hist_step;//从-PI到PI，分成36份
    float sigma1=( ((GaussianPyr[0].Octave)[SCALESPEROCTAVE].absolute_sigma) ) / (GaussianPyr[0].subsample);//SCALESPEROCTAVE+2
	int zero_pad = (int) (max<double>(3.0f, 2 * GAUSSKERN *sigma1 + 1.0f)*0.5+0.5);
...
void MySIFT::ExtractFeatureDescriptors(int numoctaves, ImageOctaves *GaussianPyr)
{
 	...
 	while(p) // 没到表尾
	{
		...
		// for (int i=0;i<128*GridSpacing;i++,i++)//我觉得是128*GridSpacing
		...
		for ( int i=0;i<512;++i,++i)
		{
			...
			for (int m=0;m<32;++m,++m)
			{
			    ...
			    x_wght[m/2] = max<double>(1 - (fabs(x - x_sample)*1.0/GridSpacing), 0);
                y_wght[m/2] = max<double>(1 - (fabs(y - y_sample)*1.0/GridSpacing), 0); 
			...
			for (int m=0;m<128;++m)
			{
				orient_wght[m] = max<double>((1.0 - 1.0*fabs(diff[m%8])/orient_bin_spacing),0);
