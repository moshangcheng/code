...
template<typename T>
class has_c_str_new : public has_c_str<T, const char* (T::*)() const> {};
...
template <typename T> typename enable_if<has_c_str<T, const char* (T::*)() const>::value, std::string>::type some_func(T t)
{
...
int main()
{
	...
	std::string lStr("abc");
	some_func(s);
	std::cout << has_c_str_new<std::string>::value << std::endl;
