...
var rssArray = [
		...
		{
			"url" : "http://feed.feedsky.com/heikezhi",
			"name" : "黑客志",
			"group" : "重要",
			"refreshTime" : "TTL",
			"networkTimeout" : "0",
...
opmlText += '<opml version="1.0">\n';
opmlText += '<head>\n';
opmlText += '<title>Feeds OPML Export</title>\n';
opmlText += '<dateCreated>' + new Date() + '</dateCreated>\n';
opmlText += '</head>\n';
opmlText += '<body>\n';
for ( var i = 0; i < rssArray.length; i++) {
	var feedTag = '<outline type="rss" version="RSS"';
	feedTag += ' text="' + rssArray[i].name + '"';
	feedTag += ' htmlUrl=""';
	feedTag += ' xmlUrl="' + rssArray[i].url + '"';
	feedTag += ' />\n';
	opmlText += feedTag;
}
opmlText += '</body>\n';
